...
func (q *counterGoroutineScheduler) Run(resolver graphql.UnitResolver, initialUnits ...*graphql.WorkUnit) []error {
	errors := q.runEnqueue(resolver, initialUnits...)

	q.wg.Wait()
	return errors
}

func (q *counterGoroutineScheduler) runEnqueue(resolver graphql.UnitResolver, units ...*graphql.WorkUnit) []error {
	atomic.AddInt64(&q.count, int64(len(units)))
	errors := make([]error, 0, len(units))
	for _, unit := range units {
		...
		go func(u *graphql.WorkUnit) {
			...
			fmt.Println("YPPP", units, unitErrors)
