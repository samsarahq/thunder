...
func (c *conn) handleSubscribe(id string, subscribe *subscribeMessage) error {
	...
	c.subscriptions[id] = reactive.NewRerunner(c.ctx, func(ctx context.Context) (interface{}, error) {
		...
		var err error
		var runMiddlewares func(index int, input *ComputationInput, output *ComputationOutput)
		runMiddlewares = func(index int, input *ComputationInput, output *ComputationOutput) {
			if index >= len(c.middlewares) {
				output.Current, output.Error = e.Execute(input.Ctx, c.schema.Query, nil, query.SelectionSet)
				return
			}
			middleware := c.middlewares[index]
			middleware(input, output, func(input *ComputationInput, output *ComputationOutput) {
				runMiddlewares(index+1, input, output)
			})
		}
		...
		runMiddlewares(0, &ComputationInput{
			Ctx:       ctx,
			Id:        id,
			Kind:      query.Kind,
			Previous:  previous,
...
func Handler(schema *Schema) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		c := CreateJSONSocket(r.Context(), socket, schema, makeCtx, &simpleLogger{})
		c.ServeJSONSocket()
	})
}
