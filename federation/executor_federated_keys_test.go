...
func createExecutorWithFederatedUser() (*Executor, *schemabuilder.Schema,*schemabuilder.Schema, *schemabuilder.Schema, error) {
	...
	*/
	...
	s2.Federation().FederatedFieldFunc("User", func(args struct{ Keys []UserKeysWithOrgId }) []*UserWithContactInfo {
	...
	*/
	...
	s3.Query().FieldFunc("privelagedUsers", func(ctx context.Context) ([]*UserWithAdminPrivelages, error) {
		users := make([]*UserWithAdminPrivelages, 0, 1)
		users = append(users, &UserWithAdminPrivelages{Id: int64(1), OrgId: int64(9086), IsAdmin: true})
		return users, nil
	})
	userWithAdminPrivelages.Federation(func(u *UserWithAdminPrivelages) *UserWithAdminPrivelages {
		return u
	})
	...
	userWithAdminPrivelages.FieldFunc("devicesWithArgs", func(ctx context.Context, user *UserWithAdminPrivelages, args struct {
			Id int64
			Temp int64
		}) ([]*DeviceWithTemperature, error) {
		devices := make([]*DeviceWithTemperature, 0, 1)
		devices = append(devices, &DeviceWithTemperature{Id: args.Id, OrgId: user.OrgId, Temp: args.Temp})
		return devices, nil
	})
...
func ExecutorQueriesBasic(t *testing.T) {
