...
func TestConnection(t *testing.T) {
	...
	inner := schema.Object("inner", Inner{}, schemabuilder.FetchObjectFromKeys(func(args struct{ Keys []*Inner }) []*Inner {
		return args.Keys
	}))
	item := schema.Object("item", Item{}, schemabuilder.FetchObjectFromKeys(func(args struct{ Keys []*Item }) []*Item {
		return args.Keys
	}))
...
func TestPaginateBuildFailure(t *testing.T) {
	...
	t.Run("key field error", func(t *testing.T) {
		...
		_ = schema.Object("item", Item{}, schemabuilder.FetchObjectFromKeys(func(args struct{ Keys []*Item }) []*Item {
			return args.Keys
		}))
	...
	t.Run("key field doesn't exist error", func(t *testing.T) {
		...
		object := schema.Object("structWithKey", StructWithKey{}, schemabuilder.FetchObjectFromKeys(func(args struct{ Keys []*StructWithKey }) []*StructWithKey {
			return args.Keys
		}))
...
func TestPaginateNodeTypeFailure(t *testing.T) {
	...
	badMethodStr := "bad method _federation on type schemabuilder.query: bad method _inner on type schemabuilder.federation:"
	_, err := schema.Build()
	if err == nil || err.Error() != fmt.Sprintf("%s federation.Item must be a struct and registered as an object along with its key", badMethodStr) {
